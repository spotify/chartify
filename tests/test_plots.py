...
def chart_color_mapper(chart_object):
    return chart_object.figure.select(bokeh.models.mappers.CategoricalColorMapper)[0]


class TestHeatmap:
    def setup_method(self):
        self.data = pd.DataFrame(
            {"category1": ["a", "a", "b", "b"], "category2": [1, 2, 1, 2], "values": [1.0, 2.0, 3.0, 4.0]}
        )

    def test_heatmap(self):
        """Area plot tests"""
        ch = chartify.Chart(x_axis_type="categorical", y_axis_type="categorical")
        ch.plot.heatmap(self.data, x_column="category1", y_column="category2", color_column="values")
        assert np.array_equal(chart_data(ch, "")["category1"], ["a", "a", "b", "b"])
        assert np.array_equal(chart_data(ch, "")["category2"], ["1", "2", "1", "2"])
        assert np.array_equal(chart_data(ch, "")["values"], [1.0, 2.0, 3.0, 4.0])
...
class TestLine:
        ...
        self.data = pd.DataFrame(
            {
                ...
                "datetimes": ["2017-01-01", "2017-01-01", "2017-01-02", "2017-01-02", "2017-01-03", "2017-01-03"],
            }
        )
    ...
    def test_single_numeric_line(self):
        ...
        assert np.array_equal(chart_data(ch, "")["number1"], [1.0, 2.0, 3.0])
        assert np.array_equal(chart_data(ch, "")["number2"], [5, 10, 0])

    def test_multi_numeric_line(self):
        """Single line test"""
        ch = chartify.Chart()
        ch.plot.line(self.data, x_column="number1", y_column="number2", color_column="category1")
        assert np.array_equal(chart_data(ch, "a")["number1"], [1.0, 2.0, 3.0])
        assert np.array_equal(chart_data(ch, "a")["number2"], [5, 10, 0])
        assert np.array_equal(chart_data(ch, "b")["number1"], [1.0, 2.0, 3.0])
    ...
    def test_single_datetime_line(self):
        ...
        data = pd.DataFrame(
            {
                "number": [1, 10, -10, 0],
                "datetimes": ["2017-01-01", "2017-01-02", "2017-01-03", "2017-01-04"],
            }
        )
        ch = chartify.Chart(x_axis_type="datetime")
        ch.plot.line(data, x_column="datetimes", y_column="number")
        assert np.array_equal(chart_data(ch, "")["datetimes"], pd.date_range("2017-01-01", "2017-01-04"))
...
class TestScatter:
    ...
    def test_multi_numeric_scatter(self):
        """Multi scatter test"""
        ch = chartify.Chart()
        ch.plot.scatter(self.data, x_column="number1", y_column="number2", color_column="category1")
        assert np.array_equal(chart_data(ch, "a")["number1"], [1.0, 2.0, 3.0])
        assert np.array_equal(chart_data(ch, "a")["number2"], [5, 10, 0])
        assert np.array_equal(chart_data(ch, "b")["number1"], [1.0, 2.0, 3.0])
        assert np.array_equal(chart_data(ch, "b")["number2"], [4, -3, -10])
...
class TestText:
    ...
    def test_single_numeric_text(self):
        ...
        ch = chartify.Chart()
        ch.plot.text(single_text, x_column="number1", y_column="number2", text_column="text")
        assert np.array_equal(chart_data(ch, "")["number1"], [1.0, 2.0, 3.0])
        assert np.array_equal(chart_data(ch, "")["number2"], [5, 10, 0])
    ...
    def test_multi_numeric_text(self):
        """Single line test"""
        ch = chartify.Chart()
        ch.plot.text(self.data, x_column="number1", y_column="number2", text_column="text", color_column="text")
        assert np.array_equal(chart_data(ch, "a")["number1"], [1.0, 2.0, 3.0])
        assert np.array_equal(chart_data(ch, "a")["number2"], [5, 10, 0])
    ...
    def test_single_datetime_text(self):
        ...
        ch = chartify.Chart(x_axis_type="datetime")
        ch.plot.text(data, x_column="datetimes", y_column="number", text_column="text")
        assert np.array_equal(chart_data(ch, "")["datetimes"], pd.date_range("2017-01-01", "2017-01-04"))
        assert np.array_equal(chart_data(ch, "")["number"], [1, 10, -10, 0])
        assert np.array_equal(chart_data(ch, "")["text"], ["a", "b", "a", "b"])
...
class CategoricalTextTest:
    def test_float_labels(self):
        label_test = pd.DataFrame(
            {
                "value": [0.20, 0.40, 0.05, 0.6, 0.2, 0.8],
                "bucket": [1, 2, 3, 1, 2, 3],
                "platform": ["android", "android", "android", "ios", "ios", "ios"],
                "value2": [1.0, 2.0, 3, 6.0, 8.0, 10.0],
            }
        )
        ch = chartify.Chart(x_axis_type="categorical")
        ch.plot.text(label_test, ["bucket", "platform"], "value", "value")
        assert np.array_equal(ch.data[0]["text_column"].values, ["0.8", "0.05", "0.6", "0.2", "0.4", "0.2"])
...
class TestAreaPlot:
    ...
    def setup_method(self):
        self.data = pd.DataFrame(
            {"category": ["a", "a", "b", "b"], "upper": [20, 30, 2, 3], "lower": [10, 20, 1, 2], "x": [1, 2, 1, 2]}
        )
    ...
    def test_single_series_zero_baseline(self):
        ...
        assert np.array_equal(chart_data(ch, None)["upper"], [20.0, 30.0, 0.0, 0.0])
    ...
    def test_multi_series_zero_baseline_unstacked(self):
        ...
        assert np.array_equal(chart_data(ch, "a")["upper"], [20.0, 30.0, 0.0, 0.0])

        assert np.array_equal(chart_data(ch, "b")["x"], [1, 2, 2, 1])
        assert np.array_equal(chart_data(ch, "b")["upper"], [2.0, 3.0, 0.0, 0.0])

    def test_multi_series_zero_baseline_stacked(self):
        """Area plot tests"""
        test_data = self.data
        ch = chartify.Chart()
        ch.plot.area(test_data, "x", "upper", color_column="category", stacked=True)
        assert np.array_equal(chart_data(ch, "a")["x"], [1, 2, 2, 1])
        assert np.array_equal(chart_data(ch, "a")["upper"], [20.0, 30.0, 0.0, 0.0])

        assert np.array_equal(chart_data(ch, "b")["x"], [1, 2, 2, 1])
        assert np.array_equal(chart_data(ch, "b")["upper"], [22.0, 33.0, 30.0, 20.0])
    ...
    def test_single_series_interval(self):
        ...
        assert np.array_equal(chart_data(ch, None)["x"], [1, 2, 2, 1])
        assert np.array_equal(chart_data(ch, None)["upper"], [20.0, 30.0, 20.0, 10.0])

    def test_multi_series_interval(self):
        """Area plot tests"""
        test_data = self.data
        ch = chartify.Chart()
        ch.plot.area(test_data, "x", y_column="upper", second_y_column="lower", color_column="category")
        assert np.array_equal(chart_data(ch, "a")["x"], [1, 2, 2, 1])
        assert np.array_equal(chart_data(ch, "a")["upper"], [20.0, 30.0, 20.0, 10.0])
    ...
    def test_multi_series_zero_baseline_unstacked_missing(self):
        ...
        assert np.array_equal(chart_data(ch, "a")["upper"], [20.0, 30.0, 40.0, 0.0, 0.0, 0.0])

        assert np.array_equal(chart_data(ch, "c")["x"], [1, 2, 3, 3, 2, 1])
        assert np.array_equal(chart_data(ch, "c")["upper"], [0.0, 0.0, 4.0, 0.0, 0.0, 0.0])
    ...
    def test_multi_series_zero_baseline_stacked_missing(self):
        ...
        assert np.array_equal(chart_data(ch, "c")["x"], [1, 2, 3, 3, 2, 1])
        assert np.array_equal(chart_data(ch, "c")["upper"], [22.0, 33.0, 44.0, 40.0, 33.0, 22.0])
...
class TestBarLollipopParallel:
    ...
    def test_vertical(self):
        ...
        for method_name in self.plot_methods:
            ...
            plot_method(sliced_data, categorical_columns="category2", numeric_column="number")
            assert np.array_equal(chart_data(ch, "")["factors"], ["2", "1", "3"])
    ...
    def test_grouping_error(self):
        for method_name in self.plot_methods:
            ...
            with pytest.raises(ValueError):
                plot_method(self.data, categorical_columns="category2", numeric_column="number")
    ...
    def test_grouped_categorical(self):
        ...
        for method_name in self.plot_methods:
            ...
            plot_method(self.data, categorical_columns=["category1", "category2"], numeric_column="number")
            assert np.array_equal(chart_data(ch, "")["number"], [10, 5, 0, 4, -3])
            ...
            ch = chartify.Chart(x_axis_type="categorical")
            plot_method = getattr(ch.plot, method_name)
            plot_method(self.data, categorical_columns=["category2", "category1"], numeric_column="number")
            assert np.array_equal(chart_data(ch, "")["number"], [5, 4, 10, -3, 0])
            multi_index = pd.MultiIndex(
                levels=[["1", "2", "3"], ["a", "b"]],
    ...
    def test_bar_color_column(self):
        sliced_data = self.data[self.data["category1"] == "a"]
        ch = chartify.Chart(x_axis_type="categorical")
        ch.plot.bar(sliced_data, categorical_columns="category2", numeric_column="number", color_column="category2")

        if version.parse(bokeh.__version__) < version.parse("3.0"):
            assert np.array_equal(chart_color_mapper(ch).factors, ["1", "2", "3"])
            assert np.array_equal(chart_color_mapper(ch).palette, ["#1f77b4", "#ff7f0e", "#2ca02c"])
            return

        vbar_glyph = ch.figure.renderers[0].glyph
        assert np.array_equal(vbar_glyph.fill_color.transform.factors, ["1", "2", "3"])
        assert np.array_equal(vbar_glyph.fill_color.transform.palette, ["#1f77b4", "#ff7f0e", "#2ca02c"])
    ...
    def test_lollipop_color_column(self):
        ...
        ch = chartify.Chart(x_axis_type="categorical")
        ch.plot.lollipop(
            sliced_data, categorical_columns="category2", numeric_column="number", color_column="category2"
        )
        ...
        assert np.array_equal(segment_glyph.line_color.transform.factors, ["1", "2", "3"])
        assert np.array_equal(segment_glyph.line_color.transform.palette, ["#1f77b4", "#ff7f0e", "#2ca02c"])
        # check circle colors
        assert np.array_equal(circle_glyph.line_color.transform.factors, ["1", "2", "3"])
        assert np.array_equal(circle_glyph.line_color.transform.palette, ["#1f77b4", "#ff7f0e", "#2ca02c"])
        assert np.array_equal(circle_glyph.fill_color.transform.factors, ["1", "2", "3"])
        assert np.array_equal(circle_glyph.fill_color.transform.palette, ["#1f77b4", "#ff7f0e", "#2ca02c"])

    def test_bar_parallel_color_column(self):
        ch = chartify.Chart(x_axis_type="categorical")
        ch.plot.parallel(self.data, categorical_columns="category1", numeric_column="number", color_column="category2")
    ...
    def test_bar_color(self):
        ...
        assert np.array_equal(ch.data[0]["color_column"], ["e", "c", "g", "d", "f"])
...
class TestBarNumericSort:
    ...
    def test_bar_sort(self):
        ch = chartify.Chart(x_axis_type="categorical")
        ch.plot.bar(
            self.data, ["x", "category"], "upper", categorical_order_by="labels", categorical_order_ascending=True
        )
        assert np.array_equal(chart_data(ch, "")["upper"], [20, 2, 30, 3, 4, 40])
        assert np.array_equal(chart_data(ch, "")["index"], [0, 1, 2, 3, 4, 5])
...
class TestBoxplot:
        ...
        for i in range(3):
            ...
            if "q2" in clean_data[i] and "q3" in clean_data[i]:
                multi_index = pd.MultiIndex.from_tuples(
                    [("a", "d"), ("a", "e"), ("b", "d"), ("c", "e")], names=("category", "sub-category")
                )

                assert clean_data[i]["factors"].equals(multi_index)
                assert clean_data[i]["factors"].equal_levels(multi_index)
                assert np.array_equal(clean_data[i]["q3"], [46, 205.25, 79, 98.5])
                assert np.array_equal(clean_data[i]["q2"], [35, 184, 68, 79.5])
            # q1-q2 boxes
                ...
                assert clean_data[i]["factors"].equal_levels(multi_index)
                assert np.array_equal(clean_data[i]["q2"], [35, 184, 68, 79.5])
                assert np.array_equal(clean_data[i]["q1"], [23, 123.5, 58, 55.75])
            # outliers
            elif "numeric" in clean_data[i]:
                multi_index = pd.MultiIndex.from_tuples(
                    [("a", "d"), ("b", "d"), ("b", "d"), ("b", "d"), ("c", "e"), ("c", "e")],
    ...
    def setup_method(self):
        ...
        random_series_2 = pd.Series(list(np.random.randint(40, 200, 50)) + list(np.random.randint(180, 225, 50)))
        df1 = pd.DataFrame({"category": "a", "sub-category": "d", "numeric": random_series_1.values})
        df2 = pd.DataFrame({"category": "a", "sub-category": "e", "numeric": random_series_2.values})

        random_series_3 = pd.Series(list(np.random.randint(50, 90, 50)) + [15, 20, 130])
        df3 = pd.DataFrame({"category": "b", "sub-category": "d", "numeric": random_series_3.values})

        random_series_4 = pd.Series(list(np.random.randint(40, 120, 50)) + [-20, 170])
        df4 = pd.DataFrame({"category": "c", "sub-category": "e", "numeric": random_series_4.values})
    ...
    def test_default(self):
        ...
        for i in range(3):
            # q2-q3 boxes
            if "q2" in clean_data[i] and "q3" in clean_data[i]:
                assert clean_data[i]["factors"].equals(pd.Index(["a", "b", "c"], name="category"))
            ...
            # q1-q2 boxes
                ...
                assert clean_data[i]["factors"].equals(pd.Index(["a", "b", "c"], name="category"))
                assert np.array_equal(clean_data[i]["q2"], [122, 68, 79.5])
                assert np.array_equal(clean_data[i]["q1"], [43.5, 58, 55.75])
            # outliers
            elif "numeric" in clean_data[i]:
                assert clean_data[i]["factors"].equals(pd.Index(["b", "b", "b", "c", "c"], name="category"))
                assert np.array_equal(clean_data[i]["numeric"], [15, 20, 130, -20, 170])

    def test_grouped_vertical(self):
        ch = chartify.Chart(x_axis_type="categorical", y_axis_type="linear")
        ch.plot.boxplot(
            self.data, ["category", "sub-category"], "numeric", color_column="category", categorical_order_by="labels"
        )
    ...
    def test_color(self):
        ...
        for i in range(3):
            if "q2" in clean_data[i] and "q3" in clean_data[i]:
                assert np.array_equal(clean_data[i]["color_column"], ["d", "e", "d", "e"])
            elif "q1" in clean_data[i] and "q2" in clean_data[i]:
                assert np.array_equal(clean_data[i]["color_column"], ["d", "e", "d", "e"])
    ...
    def test_error(self):
        ch = chartify.Chart(x_axis_type="categorical")
        with pytest.raises(ValueError):
            ch.plot.boxplot(self.data, ["category"], "numeric", categorical_order_by="count")
...
class TestAxisFormatPrecision:
    ...
    def test_axis_format_precision(self):
        ...
        for parameters, value in self.tests.items():
            assert ch.plot._axis_format_precision(parameters[0], parameters[1]) == value
...
class TestHexbin:
    def setup_method(self):
        n_samples = 2000
        np.random.seed(10)
        x_values = 2 + 0.5 * np.random.standard_normal(n_samples)
        y_values = 1 + 0.5 * np.random.standard_normal(n_samples)
    ...
    def test_hexbin(self):
        ...
        ch.plot.hexbin(self.data, "x", "y", 0.5, orientation="flattop")
        assert ch.data[0]["r"].tolist() == [-2, -1, -2, -1, -3, -2, -1, -3, -2, -4, -3]
        assert ch.data[0]["q"].tolist() == [1, 1, 2, 2, 3, 3, 3, 4, 4, 5, 5]
        assert ch.data[0]["c"].tolist() == [13, 75, 485, 232, 126, 851, 27, 123, 64, 1, 3]
        ...
        assert ch.data[0]["r"].tolist() == [-1, 0, -2, -1, 0, -3, -2, -1]
        assert ch.data[0]["q"].tolist() == [0, 0, 1, 1, 1, 2, 2, 2]
        assert ch.data[0]["c"].tolist() == [11, 3, 180, 1181, 32, 2, 391, 200]


class TestHistogram:
    def setup_method(self):
        self.data = pd.DataFrame({"values": [0, 4, 8, 22, 2, 2, 10], "dimension": ["a", "a", "a", "a", "b", "b", "b"]})
    ...
    def test_histogram(self):
        ...
        assert np.array_equal(chart_data(ch, "")["max_edge"], [11.0, 22.0])
        assert np.array_equal(chart_data(ch, "")["min_edge"], [0.0, 11.0])

    def test_grouped_histogram(self):
        """Grouped histogram."""
        ch = chartify.Chart(y_axis_type="density")
        ch.plot.histogram(self.data, values_column="values", color_column="dimension", bins=2, method="count")
        assert np.array_equal(chart_data(ch, "a")["values"], [3, 1])
        assert np.array_equal(chart_data(ch, "a")["max_edge"], [11.0, 22.0])
        assert np.array_equal(chart_data(ch, "a")["min_edge"], [0.0, 11.0])

        assert np.array_equal(chart_data(ch, "b")["values"], [2, 1])
        assert np.array_equal(chart_data(ch, "b")["max_edge"], [6.0, 10.0])
        assert np.array_equal(chart_data(ch, "b")["min_edge"], [2.0, 6.0])
...
class TestCategoricalOrderBy:
    ...
    def setup_method(self):
        ...
        self.data2 = pd.DataFrame({"category2": ["b", "a", "b", "b", "a", "c"], "number2": [1, 2, 3, 4, 5, 6]})
...
def test_categorical_axis_type_casting():
    ...
    # Multi factor test
    ch = chartify.Chart(blank_labels=True, x_axis_type="categorical")
    ch.plot.bar(test_df, categorical_columns=["categorical_strings", "categorical_integers"], numeric_column="numeric")
    ...
    # Stacked bar test
    ch = chartify.Chart(blank_labels=True, x_axis_type="categorical")
    ch.plot.bar_stacked(test_df, "categorical_strings", "numeric", "categorical_floats")
