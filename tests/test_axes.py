...
class TestBaseAxes:
    ...
    def test_datetime_set_xaxis_range(self):
        ch = chartify.Chart(x_axis_type="datetime")
        ch.axes.set_xaxis_range(pd.to_datetime("2018-01-01"), pd.to_datetime("2018-02-01"))
    ...
    def test_xaxis_tick_orientation(self):
        ...
        quantity_by_fruit_and_country = (
            data.groupby(["fruit", "country", "date"])["quantity"].sum().reset_index().head(10)
        )
        ...
        assert np.allclose(ch.figure.xaxis[0].major_label_orientation, 1.5707963267948966)
        assert ch.figure.xaxis[0].subgroup_label_orientation == "parallel"
        assert np.allclose(ch.figure.xaxis[0].group_label_orientation, 0.7853981633974483)
        ...
        assert np.allclose(ch.figure.xaxis[0].major_label_orientation, 1.5707963267948966)
        assert np.allclose(ch.figure.xaxis[0].subgroup_label_orientation, 1.5707963267948966)
        assert np.allclose(ch.figure.xaxis[0].group_label_orientation, 1.5707963267948966)
        ...
        ch.axes.set_yaxis_tick_orientation(["vertical", "vertical", "diagonal"])
        assert np.allclose(ch.figure.yaxis[0].major_label_orientation, 1.5707963267948966)
        assert ch.figure.yaxis[0].subgroup_label_orientation == "parallel"
        assert np.allclose(ch.figure.yaxis[0].group_label_orientation, 0.7853981633974483)
